...
export const SENSITIVE_ENV_KEYS = [
  ...
  // Не «ключ», а ДОМАШНЯ АДРЕСА: точні координати села на дві тисячі людей.
  // Заради цього змінну взагалі й винесли з config.yml — тримати її в оточенні
  // дочірнього claude, який читає тіла листів і RSS вакансій, означало б
  // повернути ту саму витік іншим шляхом. Самому оркестраторові вона потрібна
  // на старті (loadConfig), тобто до й незалежно від спавна CLI.
  'OWNER_LOCATIONS',
] as const;
...
function runClaude(
  ...
  return new Promise((resolve, reject) => {
    ...
    // ⚠️ Обробник ПЕРЕД записом, не після. CLI, що вийшов раніше, ніж ми
    // дописали промпт (битий токен, миттєвий crash), закриває свій stdin — і
    // запис у нього кидає EPIPE. Без слухача це unhandled rejection: процес
    // Node падає з іншою, неінформативною причиною, ховаючи справжню — ту, що
    // вже їде в reject нижче через exit code. Ковтаємо саме тут: сам збій
    // однаково буде повідомлений через 'close'/'error'.
