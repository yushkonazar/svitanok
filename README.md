...
flowchart TB
    ...
    SCHED -.->|"той самий guard, дубль неможливий"| O
    O --> CLI
    ...
    W -->|"GET /api/status — ЄДИНИЙ маршрут без auth"| Badge["бейдж «живий сервіс» на yushko.dev"]
...
4. **Публічний маршрут рівно один.** `GET /api/status` віддає одне поле — мітку
   часу останнього брифінгу — з окремого KV-ключа. Решта `/api/*` закрита
   HMAC-перевіркою `initData` власника.
