...
> Локації погоди: **Львів і Немовичі**.
>
...
| weather       | producer       | погода по **кількох локаціях** (Львів, Немовичі); пише в RunBus |
| calendar      | producer       | події Google Calendar на сьогодні; пише в RunBus                |
| today         | consumer       | синтез weather+calendar одним рядком                            |
| news          | consumer (LLM) | топ 2-3 на категорію, дедуп, адаптивний фільтр 👍/👎            |
...
Конфіг містить **список** локацій (Львів, Немовичі). Модуль фетчить кожну,
...
людяно (напр. «Львів +12°, Немовичі +9° ☔»), без дублювання з блоком weather (§9).
...
7. Кнопки `Більше: <категорія>`; `why` — в expandable. 👍/👎 (фаза B) → §6.1.

#### 6.1 preferenceWeights

Вага на категорію в `[0.5, 2.0]`, старт `1.0`. 👍: `+0.15` (max 2.0); 👎: `-0.15`
(min 0.5). Decay раз на тиждень: `w += (1.0 - w) * 0.1`. Вага множить важливість,
не занулює.
...
locations:
  ...
  - { lat: 51.1200, lon: 26.4600, name: 'Немовичі' } # ⚠️ звірити координати
...
**Фаза B (інтерактив):** Cloudflare Worker як webhook. Команди/кнопки, 👍/👎.
...
> (двофазний) + weather (дві локації, Львів+Немовичі) end-to-end з `--dry-run`.
...
### 19.13 Захист main vs щоденний коміт стану

Бот щодня пушить `state.json`; захищений `main` (лише через PR) це блокує. Стан
комітиться в окрему незахищену гілку **`state`**, `main` лишається захищеним і
лінійним (§4.3).
