...
//
// НАВІЩО тут, а не в web/app.
//
// ⚠️ Стара причина ЗАСТАРІЛА і була тут ще довго після того, як перестала бути
// правдою: «у web/app немає ні тест-раннера, ні лінтера». Обидва є — власний
// vitest (9 файлів) і eslint із react-hooks на 'error', і CI ганяє їх окремою
// джобою `dashboard`. Коментар, що описує світ дворічної давнини, шкідливіший
// за його відсутність: наступний читач зробить із нього хибний висновок.
//
// Справжня причина інша й лишається чинною: корінь і web/app мають ОКРЕМІ
// інстали zod, і саме КОРЕНЕВИЙ тест зводить реальний aggregateStats зі
// схемою — тобто ловить розбіжність між сервером і контрактом, чого тест
// усередині web/app зробити не може (він не бачить stats-core.mjs).
//
// Ставки при цьому ті самі: якщо safeParse провалиться, вкладка падає в
// «Формат змінився», а смоуком це не ловиться — у демо client.ts віддає SAMPLE
// НЕ парсячи, тож схема працює тільки в реальному Telegram.
//
