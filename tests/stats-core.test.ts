...
describe('stats-core — сон (Блок «Сон»)', () => {
  ...
  it('open НЕ чіпає ніч, де wokeAt уже проставлено (ідемпотентно — не лише перше відкриття доби)', () => {
    let s = emptyStore();
    s = recordEvent(s, { type: 'sleepStart' }, '2026-07-10', null, '2026-07-10T23:47:00.000Z');
    s = recordEvent(s, { type: 'open' }, '2026-07-11', 30, '2026-07-11T07:52:00.000Z');
  ...
  it('РЕГРЕСІЯ (фідбек власника): пізній передсонний open того ж календарного дня БІЛЬШЕ НЕ блокує ранкове закриття ночі', () => {
    // Відтворює реальний кейс з прод-KV: власник тапнув «Ліг спати» вже ПІСЛЯ
    // півночі за Києвом (checkinDateKey зсуває годину на попередній день), але
    // ДО ЦЬОГО встиг відкрити застосунок того ж календарного дня (01:38) —
    // старий код гейтив закриття ночі через firstOpenToday, тож той ранній
    // open «з'їдав» прапорець, і РЕАЛЬНЕ ранкове відкриття (10:00, той самий
    // календарний день) більше не рахувалось «першим» — wokeAt не проставлявся,
    // чек-ін не автозаповнювався.
    let s = emptyStore();
...
describe('stats-core — розширені метрики (A2)', () => {
  ...
  it('heatmap: вікно НЕ сягає раніше першого реального запису (фідбек власника, прод: 12-тижневе вікно захоплювало тижні до запуску Світанку)', () => {
    let s = emptyStore();
    s = recordEvent(s, { type: 'open' }, '2026-07-06'); // перший запис у сторі — той самий тиждень
  ...
  it('habitWeekly/interestsTrend: вікно росте потижнево від першого запису, не стрибає одразу на максимум (фідбек власника)', () => {
    let s = emptyStore();
    s = recordEvent(s, { type: 'open' }, '2026-07-06');
    s = recordEvent(s, { type: 'news_click', category: 'Наука' }, '2026-07-06');
    const stats1 = aggregateStats(s, '2026-07-06');
    expect(stats1.habitWeekly).toHaveLength(1);
    expect(stats1.interestsTrend.weeks).toHaveLength(1);

    // Тиждень по тому — вікно виросло до 2, не «стрибнуло» одразу на 12/26.
    s = recordEvent(s, { type: 'open' }, '2026-07-13');
    const stats2 = aggregateStats(s, '2026-07-13');
    expect(stats2.habitWeekly).toHaveLength(2);
    expect(stats2.interestsTrend.weeks).toHaveLength(2);
  });
