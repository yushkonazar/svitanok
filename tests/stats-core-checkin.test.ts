...
describe('sleepKind — режим ночі виводить тривалість і якість', () => {
  ...
  /* ⚠️ ПРАВИЛО ПЕРЕВЕРНУТО, і це виправлення, а не зміна смаку.
     Спершу тут стояло «власна відповідь головніша за виведену» — з думкою про
     легасі-доби. Але легасі-доба не має sleepKind ВЗАГАЛІ (тест нижче), тобто
     те правило захищало неіснуючий випадок, а натомість покривало реальний і
     шкідливий: послідовність «Спав → 8 годин → передумав, Не спав» лишала в KV
     sleepH=8 поряд із sleepKind='none', і модель читала безсонну ніч як вісім
     годин сну — рівно те, заради чого поле й додавалось.

     Тепер режим ночі — головніший: він і є найсвіжіша відповідь власника. */
  it('режим ночі перекриває залишок від попереднього вибору', () => {
    const f = flat({ sleepKind: 'naps', sleepH: 4.5, sleepQ: 3 });
    expect(f.sleepH).toBe(2);
    expect(f.sleepQ).toBe(2);
  });

  it('«не спав» перекриває навіть повністю заповнений сон', () => {
    const f = flat({ sleepKind: 'none', sleepH: 8, sleepQ: 5 });
    expect(f.sleepH).toBe(0);
    expect(f.sleepQ).toBe(1);
  });
