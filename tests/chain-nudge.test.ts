...
// і наступне через 15; друге - кінець (nudge стирається); клейм проти
// подвійного тіку; кілька ланцюгів в одну адресу - одним повідомленням;
// softWaitingLine - раз на київський день після доби тиші.
...
function setup() {
  ...
  const sent: { chat_id: string; thread?: number; text: string }[] = [];
  vi.stubGlobal(
    'fetch',
...
describe('chainNudgeTask', () => {
  it('до строку - нічого; +5 → нагадування і наступне через 15; друге → nudge стерто, третього немає', async () => {
    ...
    // Той самий тік ще раз - клейм не дає дубля.
  ...
  it('claimNudge - CAS за n: два тіки з тим самим due-списком - лише один клейм', async () => {
    const { env, seed } = setup();
    const c = { id: 'c1', n: 0 };
    seed('c1', { awaiting: 'venue', nudge: { at: new Date(NOW).toISOString(), n: 0 } });
    expect(await claimNudge(env, c, NOW)).toBe(true);
    expect(await claimNudge(env, c, NOW)).toBe(false);
    expect(await claimNudge(env, { id: 'c1', n: 1 }, NOW)).toBe(true);
  });
  ...
  it('без DB - пропуск; DM-адреса (thread_id dm) - без message_thread_id', async () => {
    const { env, sent, seed } = setup();
...
describe('softWaitingLine', () => {
  ...
  it('кілька старих - одним рядком через кому', async () => {
    ...
    expect(await softWaitingLine(env, NOW)).toBe('Ланцюги столиків чекають вибору: Креденс, Дім.');
  });
});
