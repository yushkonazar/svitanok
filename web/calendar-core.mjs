 ...
 *
 * Кап (рев'ю CC1): діапазон до 7 днів може дати десятки подій — а цей текст іде
 * в транскрипт РЕЗУЛЬТАТОМ кроку readCalendar і лишається там до кінця прогону.
 * Без обмеження один виклик зʼїдає стелю MAX_PROMPT_LEN, яку ділять результати
 * усіх наступних кроків, а переповнення ріже середину транскрипту. Тому
 * ≤MAX_RANGE_EVENTS подій і ≤MAX_RANGE_LEN символів; надлишок -> «…(ще N)».
