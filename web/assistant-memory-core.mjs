...
// РАЗОМ із own-data дайджестом (до MAX_DIGEST_LEN) і календарем — тому тримаємо
// коротко: ≤MAX_HISTORY_TURNS реплік, кожна ≤MAX_TURN_LEN, рендер
// ≤MAX_RENDER_LEN. Тісно не через стелю хоста (MAX_PROMPT_LEN підняли
// 11.08.2026), а тому що це лише ПОЧАТОК транскрипту: далі хост дописує
// результат кожного кроку, і при переповненні ріже середину (ревʼю CM).
