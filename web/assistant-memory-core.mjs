...
// «її» = попередня подія). Без I/O — Worker читає/пише canonical
// AssistantHistoryDO; KV `assistantHistory` — лише compatibility mirror, не
// конкурентний state-блоб.
//
// Ключовий інваріант бюджету: історія — це те, з ЧОГО транскрипт починається
// (разом із текстом користувача, agent-runtime.mjs). Own-data дайджест, календар
// і пошта поруч із нею на старті НЕ лежать — вони дописуються пізніше,
// результатами кроків. Тому тримаємо коротко: ≤MAX_HISTORY_TURNS реплік, кожна
// ≤MAX_TURN_LEN, рендер ≤MAX_RENDER_LEN. Тісно не через стелю хоста
// (MAX_PROMPT_LEN підняли 11.08.2026), а тому що ГОЛОВА транскрипту переживає
// весь прогін: при переповненні clipAgentTranscript ріже середину, а голову
// лишає — тож роздута історія займає місце до останнього кроку й ніколи не
// витісняється (ревʼю CM).
...
/**
 * TTL контексту `assistantHistory` (аудит §KV: «жоден ключ не має TTL»).
 *
 * Це «стільки ТИШІ», а не «стільки життя»: кожен append пересуває DO alarm,
 * тож розмова живе, доки нею користуються, а покинута — зникає сама. Навіщо
 * взагалі: тут осідають теми листів і назви
