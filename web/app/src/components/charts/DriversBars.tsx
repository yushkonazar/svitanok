...
export function DriversBars({ drivers }: { drivers: Stats['checkinModel']['drivers'] }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <Hint>
        Наскільки оцінка дня відрізняється між добами, де показник був високий, і тими, де низький.
        Довжина смуги — СИЛА впливу (не просто різниця середніх), число праворуч — на скільки балів
        зсувається день. «Витримує поправку» означає, що ефект лишається помітним навіть з
        урахуванням того, що показників тут десятки: при такій кількості перевірок один-два
        «відкриття» трапляються чисто випадково, і поправка їх відсіює. Тьмяні смуги — даних поки
        замало, щоб їм вірити.
      </Hint>
    </div>
  );
}
