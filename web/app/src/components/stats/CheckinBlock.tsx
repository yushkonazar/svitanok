    ...
    <div className="flex flex-col gap-3">
      ...
      {quad.n > 0 && (
        <Card>
          ...
          <Hint>
            ...
            відповідають на те, чого не скаже жодне окреме число: важкий день і безрезультатний день
            — не одне й те саме. Пороги фіксовані (4-5 високо, 1-2 низько), а не «відносно твоїх
            звичайних днів»: інакше доба, яку ти бачив як потік, через місяць мовчки стала б тихим
            днем — просто тому, що зсунулась медіана.
          </Hint>
        </Card>
      )}
        ...
        <Card>
          ...
          {nights.effect.ready ? (
            ...
            <div className="mt-2 text-[11px] text-tx3">
              Порівняти з рештою днів ще рано: {nights.effect.nRough} із {nights.effect.needed ?? 8}{' '}
              потрібних діб з оцінкою.
            </div>
          )}
          <Hint>
            Ніч без сну й ніч уривками доти впливали лише на «Індекс дня» — числом, але не словами.
            Причина тут важливіша за факт: чекав ранку, доробляв проєкт і просто не міг заснути — це
            три різні ночі з різними висновками.
          </Hint>
        </Card>
      )}
      ...
      {filledDays > 1 && (
        <Card>
          ...
          <Hint>
            Твій СЕРЕДНІЙ день від ранку до вечора. Показує те, чого середнє число сказати не може:
            ти згоряєш надвечір чи навпаки розганяєшся. Лінія вниз — енергія витікає до вечора.
          </Hint>
        </Card>
      )}
      ...
      <Card>
        ...
        <Hint>
          Та сама сітка 5×5, по якій ти тапаєш у чек-іні. Число в клітинці — скільки разів ти в ній
          опинявся. Праворуч-угорі — бадьорий і в настрої, ліворуч-унизу — виснажений. Перемикач
          зверху розділяє ранок, день і вечір: це різні стани з різними причинами, і разом вони
          змішувались в одну купу.
        </Hint>
      </Card>
      ...
      {model.archetypes.ready && (
        <Card>
          ...
          <Hint>
            «Середнього дня» не існує — натомість твої доби згруповані в кілька типів. Кожна фігура
            — форма одного типу за пʼятьма вимірами, відсоток — як часто такі дні трапляються. Чим
            більший промінь, тим сильніший вимір.
          </Hint>
        </Card>
      )}
      ...
      {laggedEntries.length > 0 && (
        <Card>
          ...
          <Hint>
            Єдине місце, що дивиться на ЗАВТРАШНІЙ день: чи сьогоднішнє відновлення й рух повʼязані
            з тим, як мине наступна доба. ρ — сила звʼязку від −1 до +1 (0 — звʼязку немає). «Шум?»
            означає, що вибірки поки замало, щоб вірити числу.
          </Hint>
        </Card>
      )}
      ...
      {detailsOpen && (
        <div className="flex flex-col gap-3 border-t border-glassb pt-3">
          ...
          {(tops.blockers.length > 0 || tops.helpers.length > 0) && (
            <Card>
              <SubLabel>
                ЩО ЗАВАЖАЛО І ЩО ПОМАГАЛО · {daysWindowLabel(tops.days, tops.filled)}
              </SubLabel>
              ...
              <Hint>
                Скільки діб ти обирав кожен варіант. «Нічого» не рахується — це свідома відповідь, а
                не причина. Рядок «жодного разу» показує варіанти, які за все вікно не обрано — це
                підстава прибрати їх зі списку, коли вони простоять порожніми досить довго.
              </Hint>
            </Card>
          )}
          ...
          {social.tops.length > 0 && (
            <Card>
              <SubLabel>
                СОЦІАЛЬНИЙ КОНТЕКСТ · {daysWindowLabel(social.days, social.filled)}
              </SubLabel>
              <div className="mt-2">
                <RankedBars
          ...
          {cat.total >= 5 && (
            <Card>
              ...
              <Hint>
                Куди реально йде час: скільки діб кожна категорія забирала день. «Оцінка» — середня
                оцінка дня в таких добах, тобто які заняття корелюють із хорошим днем; зʼявляється
                лише від 4 оцінених діб.
              </Hint>
            </Card>
          )}
          ...
          {drift.total >= 5 && drift.pct != null && (
            <Card>
              <SubLabel>
                ПЛАН ПРОТИ РЕАЛЬНОСТІ · {daysWindowLabel(drift.days, drift.total)}
              </SubLabel>
              ...
              <Hint>
                ...
                зіставлені. Число — СЕРЕДНЯ ЧАСТКА виконаного плану: два планові пункти й один
                зроблений дають 50%, а не 100%. Поруч — скільки діб пішли за планом повністю. Пари
                внизу — найчастіші підміни, тобто куди насправді витікає час; пара, що трапилась
                один раз, у список не потрапляє.
              </Hint>
            </Card>
          )}
          ...
          {cal.ready ? (
            <Card>
              ...
              <Hint>
                Ранкове «яким очікуєш день» проти вечірньої оцінки. Число — СЕРЕДНІЙ зсув: додатний
                означає, що дні виходять кращими за прогноз. Три кошики поруч не зайві: нульовий
                зсув буває і коли щодня влучаєш, і коли половина днів краща, а половина гірша — це
                різні історії. Рахується від {cal.n} діб, де є обидві відповіді.
              </Hint>
            </Card>
          ) : (
            cal.n > 0 && (
              <Card>
                ...
                <Note>
                  Потрібно {cal.needed} діб, де є і ранкове очікування, і вечірня оцінка — зараз{' '}
                  {cal.n}. На меншій вибірці «ти песиміст» було б монеткою.
                </Note>
              </Card>
            )
          )}
          ...
          {mv.ready ? (
            <Card>
              ...
              <Hint>
                ...
                виконаним, коли факт не НИЖЧИЙ за план: планував легкий рух, вийшло тренування — це
                виконано. Другий рядок про протилежне: скільки разів рух стався там, де його не
                планував. Зводити обидва в один відсоток означало б втратити половину картини.
              </Hint>
            </Card>
          ) : (
            mv.n > 0 && (
              <Card>
                ...
                <Note>
                  Потрібно {mv.needed} діб, де є і ранковий намір, і вечірній факт — зараз {mv.n}.
                </Note>
              </Card>
            )
          )}
