    ...
    <div className="flex flex-col gap-3">
      ...
      {open && isError && (
        <div className="text-[11px] leading-[1.5] text-tx3">
          Не вдалося прочитати історію. Це окреме сховище — решта статистики від цього не залежить.
        </div>
      )}
      ...
      {open && enough && (
        <div className="flex flex-col gap-3.5">
          ...
          <Hint>
            {months!.length < MIN_MONTHS_FOR_LINE
              ? 'Поки місяців менше за ' +
                MIN_MONTHS_FOR_LINE +
                ', показані числа, а не лінія: двома-трьома точками тренду не буває, а відрізок між ними виглядав би саме як тренд. '
              : ''}
            Місячні середні з окремого холодного сховища — саме тому цей блок бачить далі за решту
            екрана, яка живе на 30–90 добах. Порожній місяць дає розрив у лінії, а не падіння в
            нуль. Уже записаний місяць більше не перераховується: його сирі доби з часом виходять за
            межі зберігання, і перерахунок дав би гірше число, ніж те, що збережено.
          </Hint>
        </div>
      )}
    </div>
