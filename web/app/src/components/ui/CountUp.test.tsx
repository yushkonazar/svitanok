 ...
 *
 * ⚠️ Позначка ВІДНОСНА до performance.now(), а не абсолютні 10_000. Хук бере
 * t0 = performance.now() на старті анімації й рахує прогрес як (t − t0). Тобто
 * щойно процес vitest прожив довше за 10 секунд до цього файлу — а на холодному
 * прогоні чи повільній CI-машині це норма, — фіксована позначка опинялась
 * ПОЗАДУ t0, прогрес виходив відʼємним, і число лишалось нулем.
 *
 * Тест падав із «expected '0' to be '42'» при цілком правому коді, і ловився
 * лише на повільному прогоні: локально в теплому кеші файл стартує на другій
 * секунді й розбіжності не видно. Класична міна, що вибухає в CI.
 */
const flush = (t = performance.now() + 10_000) =>
  act(() => {
