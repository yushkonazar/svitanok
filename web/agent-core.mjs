...
export const ASSISTANT_ACTION_SCHEMA = {
  ...
  properties: {
    ...
    // полів — компроміс під тодішні 2000 символів MAX_SCHEMA_LEN, який не давав
    // скласти в один батч два readMail з різними запитами. Межу відтоді
    // підняли, тож тримати цей компроміс немає причин.
    reads: {
      type: 'array',
      items: {
        type: 'object',
 ...
 * СТИСЛИМ — хост відхиляє промпт, довший за MAX_SYSTEM_PROMPT_LEN. Саме число —
 * у host/llm-host-core.mjs і тут навмисно не дублюється: межу вже піднімали, і
 * копії в коментарях протухли (тест limit-comment-parity стереже цей клас).
 * Тест довжини у tests/agent-core.test.ts стереже саму межу (виміряно по всіх
 * 7 днях тижня — weekday:'long' дає різну довжину).
 ...
 * обрізає РЕПЛІКУ по MAX_TURN_LEN з ХВОСТА («…»), тож маркер у кінці на
