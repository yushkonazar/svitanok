...
export const ACTION_LEVELS = {
  ...
  'style.collect': 'T1',
  // Документ перестає бути доказом для наступних відповідей одразу після
  // revoke, але його ще можна перевірено видалити T2 окремим рішенням.
  // Імпорт явного файла Drive не є публікацією, але додає новий приватний
  // корпус, який впливатиме на майбутні відповіді, тому власник бачить T1.
  'knowledge.import': 'T1',
  'knowledge.revoke': 'T1',
  // T2 - ✅ + слово, TTL 10 хв
  forget: 'T2',
