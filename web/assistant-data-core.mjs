...
// сьогоднішній брифінг) у компактний текст для LLM-промпту. Дайджест — це
// РЕЗУЛЬТАТ дії readOwnData: хост дописує його в transcript (user-prompt),
// бюджет якого — MAX_PROMPT_LEN (окремий від системного промпту з його
// MAX_SYSTEM_PROMPT_LEN; обидва числа живуть у host/llm-host-core.mjs і тут
// навмисно не дублюються) — тому кап MAX_DIGEST_LEN лишає запас під результати
// решти кроків. Без I/O — Worker читає KV (loadState/loadStats/`latest`) і
...
// заміщає транскрипт, а ДОПИСУЄТЬСЯ до нього (`currentTranscript += append`,
// agent-loop-core.mjs) — і лишається там до кінця прогону, поряд із результатами
// решти кроків (пошта, календар, тіла листів). Тобто стелю доїдає не одне
// повідомлення, а СУМА результатів за прогін: кап одного результату треба
// множити на AGENT_MAX_STEPS, а не порівнювати зі стелею наодинці. При
// переповненні clipAgentTranscript ріже СЕРЕДИНУ (голову й хвіст лишає), тобто
// губиться саме те, що агент прочитав дорогою.
