...
// transcript (user-prompt), чий бюджет на хості — MAX_PROMPT_LEN (окремий від
// системного промпту з його MAX_SYSTEM_PROMPT_LEN; обидва числа живуть у
// host/llm-host-core.mjs і тут навмисно не дублюються) — тому кап
...
// підняли 11.08.2026), і кап лишається тісним з іншої причини: цей текст — лише
// ПОЧАТОК транскрипту, який далі росте на хості, дописуючи результат кожного з
// AGENT_MAX_STEPS кроків. При переповненні clipAgentTranscript ріже СЕРЕДИНУ
// (лишає голову й хвіст), тобто губиться саме те, що агент прочитав дорогою.
// Бюджет початкового повідомлення ділиться між історією (500), календарем (900),
// поштою (900), текстом користувача (500) і цим дайджестом.
