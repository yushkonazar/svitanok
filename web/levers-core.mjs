 ...
 *
 * Стеля даних: `checkins`/`days` живуть 365 діб (CHECKIN_CAP / DAYS_CAP), тож
 * глибше просто немає з чого рахувати. Наслідок — вікно КОТИТЬСЯ: коли історія
 * переросте рік, найдавніші тижні випадатимуть. Для «що на що тягне ЗАРАЗ» це
 * радше правильно, ніж ні: важіль трирічної давнини вже не важіль.
 *
 * ⚠️ 51, А НЕ 52 — і це не округлення. Понеділок першого кошика має вік
 * (today − понеділок цього тижня) + 7·(N−1) + 7 діб, тобто при N=52 від 364 до
 * 370. Штатний прогін у понеділок дає 364 — усі сім діб живі; але гейт крону —
 * `weekOf`, а не день тижня, тож пропущений понеділок (збій, деплой, пауза
 * крону) зсуває прогін на середину тижня, і перші доби кошика виявляються
 * старшими за кап — уже видалені. Тиждень виходить систематично занижений,
 * тобто рівно той дефект, від якого береже правило «поточний тиждень не
 * входить», лише на іншому кінці ряду. Один тиждень запасу знімає питання.
 */
export const LEVERS_WEEKS_WINDOW = 51;
...
export function buildWeeklySeries(store, state, todayKey, weeks) {
  ...
  /**
   * Найраніший тиждень вікна, у якому стор ВЗАГАЛІ ВЕВСЯ.
   *
   ...
   *
   * ⚠️ МЕЖУ СТАВЛЯТЬ ЛИШЕ `checkins` І `days` — два ДОБОВІ записи, сама
   * наявність яких доводить, що стор тоді вже вівся. Журнали подій
   * (`appliedLog`, `funnelMeta`, `roadmapProgress`) її НЕ ставлять, і це не
   * дрібниця: `roadmapProgress` живе в іншому блобі, не має капа й старший за
   * стор `stats`. Перша версія цієї межі була одна на всіх, і ОДИН запис
   * роадмепу 47-тижневої давнини повертав 22 фальшиві нулі одночасно в `opens`
   * і `applied` (modeShare 0.458 — гейт знову мовчить). Після появи стору нуль
   * у журналі подій означає «події не було» і лишається чесним.
   */
  ...
  for (const [d, rec] of Object.entries(checkins)) {
    ...
    // ⚠️ Межу ставимо ДО перевірки `recorded`: навіть порожній запис чек-іну
    // доводить, що стор тоді вже вівся, — а межа саме про це, не про якість
    // конкретної доби.
    seen(i);
